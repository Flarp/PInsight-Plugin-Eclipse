 ...
 * @since 2.0
 */
public class PInsightAnalysisModule extends TmfStateSystemAnalysisModule {
    ...
    protected ITmfStateProvider createStateProvider() {
        return new PInsightStateProvider(checkNotNull(getTrace()));
    }

    @Override
    public boolean setTrace(ITmfTrace trace) throws TmfAnalysisException {
        if (!(trace instanceof PInsightTrace)) {
            return false;
        }
    ...
    public boolean canExecute(ITmfTrace trace) {
        ...
        PInsightTrace ustTrace = (PInsightTrace) trace;
        ...
        /* ... taken with UST >= 2.8 ... */
        if (!PInsightTrace.TRACER_NAME.equals(tracerName)) {
            return false;
        }
        if (majorVersion < 2) {
            return false;
    ...
    public Collection<UstDebugInfoBinaryFile> getAllBinaries() {
        ...
        try {
            for (Integer baddrQuark : baddrQuarks) {
                int buildIdQuark = ss.getQuarkRelative(baddrQuark, PInsightStateProvider.BUILD_ID_ATTRIB);
                int debugLinkQuark = ss.getQuarkRelative(baddrQuark, PInsightStateProvider.DEBUG_LINK_ATTRIB);
                int pathQuark = ss.getQuarkRelative(baddrQuark, PInsightStateProvider.PATH_ATTRIB);
                int isPICQuark = ss.getQuarkRelative(baddrQuark, PInsightStateProvider.IS_PIC_ATTRIB);
    ...
    public @Nullable UstDebugInfoLoadedBinaryFile getMatchingFile(long ts, long vpid, long ip) {
        try {
            ...
            final int memszQuark = ss.getQuarkRelative(baddrQuark, PInsightStateProvider.MEMSZ_ATTRIB);
            ...
            final int pathQuark = ss.getQuarkRelative(baddrQuark, PInsightStateProvider.PATH_ATTRIB);
            String filePath = state.get(pathQuark).getStateValue().unboxStr();

            final int buildIdQuark = ss.getQuarkRelative(baddrQuark, PInsightStateProvider.BUILD_ID_ATTRIB);
            ITmfStateValue buildIdValue = state.get(buildIdQuark).getStateValue();
            String buildId = unboxStrOrNull(buildIdValue);

            final int debugLinkQuark = ss.getQuarkRelative(baddrQuark, PInsightStateProvider.DEBUG_LINK_ATTRIB);
            ITmfStateValue debugLinkValue = state.get(debugLinkQuark).getStateValue();
            String debugLink = unboxStrOrNull(debugLinkValue);

            final int isPicQuark = ss.getQuarkRelative(baddrQuark, PInsightStateProvider.IS_PIC_ATTRIB);
